...
volbar:connect_signal("button::press", function(_, _, _, button)   -- Overwrites the button press behaviour to open pavucontrol when clicked
        if (button == 3) then awful.spawn('pavucontrol --tab=3', false)
        end
    end
)
...
function theme.at_screen_connect(s)
    ...
    local l_separator = wibox.widget.textbox(" \\ ")
    local r_separator = wibox.widget.textbox(" / ")
