...
local GET_SPOTIFY_STATUS_CMD = 'playerctl --player=spotifyd status'
local GET_CURRENT_SONG_CMD = 'playerctl --player=spotifyd current'
...
local function worker(args)
    ...
    spotify_widget:connect_signal("button::press", function(_, _, _, button)
        if (button == 1) then
            awful.spawn.with_shell("playerctl --player=spotifyd play", false)      -- left click
        elseif (button == 4) then
            awful.spawn.with_shell("playerctl --player=spotifyd next", false)  -- scroll up
        elseif (button == 5) then
            awful.spawn.with_shell("playerctl --player=spotifyd prev", false)  -- scroll down
        end
